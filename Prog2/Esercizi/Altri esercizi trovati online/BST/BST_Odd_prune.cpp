...
sia in grado di stampare tutti i nodi della struttura secondo una visita preorder
supponendo che ad ogni nodo dell'albero sia stato tagliato ogni suo figlio avente come parte intera della sua chiave un valore pari.
*/
#include <iostream>
#include <fstream>
using namespace std;
...
private:
	...
	void _preorder(ostream& os, Nodo<T>* r,Nodo<T>* tmp){
		if(!r) return;
		if(r == root || int(r->getKey())%2 != 0){
			os << r->getKey() << " ";
			_preorder(os, r->getLeft(),tmp);
			_preorder(os, r->getRight(),tmp);
		}
	}
