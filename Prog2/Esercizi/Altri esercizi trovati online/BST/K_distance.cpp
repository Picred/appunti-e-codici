...
/*
Un nodo x dell'albero di dice a distanza K se 
il numero di archi nel cammino discendente dalla radice al nodo x 
è esattamente pari a K.

###calcolare, dato una distanza K, 
l'insieme dei nodi dell'albero a distanza K dalla radice
*/
...
private:
	...
	void _altezza(ostream& os,Nodo<T>* r, int n){
		if(!r) return;
		Nodo<T>* tmp = _min(root);
		int p = 0;
		while(tmp){
			int t = _prof(tmp->getKey(),r);
			if(t==n) os << tmp->getKey() << " ";
			tmp = _successore(tmp);
		}
	}
